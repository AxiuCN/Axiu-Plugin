...
**原神深渊排行** — 通过 hook miao-plugin 的 MysApi 自动采集数据，不抢 `#深渊`/`#剧诗`/`#幽境` 命令（查询仍走 miao-plugin）。仅注册排行命令：

| 命令 | 权限 | 说明 |
|------|------|------|
| `#深渊排名` / `#深境排行` | all | 层数→星数→战斗次数，每行显示层、★、战斗 |
| `#剧诗排名` / `#幻想排行` | all | 模式→幕数→用时→借出，每行显示模式、幕、用时、借出 |
| `#幽境排名` / `#危战排行` | all | 难度→用时，每行显示难度、用时、徽章（仅展示） |
| `#深渊排名 星数` | all | 指定维度排行（支持 `层数`/`星数`/`战斗次数`/`模式`/`借出次数`/`难度`） |
| `#重置深渊排名` | master | 重置本群该类型排行数据 |
| `#开启深渊排行` / `#关闭深渊排行` | master | 开关本群排行功能 |

- 幽境危战徽章（含虹彩徽章）在排行页展示但不参与排序
- 数据 90 天自动过期，Redis ZSET 多维度加权复合排序
